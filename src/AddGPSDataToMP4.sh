...
for file in "$pathToVideoDirectory"/*.{SRT,txt}; do
    ...
    if [[ -f "$file" ]]; then
        ...
        exiftool -overwrite_original -P -Keys:GPSCoordinates="$GPSLatitude, $GPSLongitude, $GPSAltitude" -ItemList:GPSCoordinates="$GPSLatitude, $GPSLongitude, $GPSAltitude" $pathToVideoDirectory/$fileNameWithoutType.mp4
